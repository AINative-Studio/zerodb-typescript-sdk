...
import {
  ...
  CreateTaskSequenceResponse
} from '../types/agents';

export class AgentOrchestrationOperations {
  ...
  async createTask(request: CreateTaskRequest): Promise<CreateTaskResponse> {
    return this.client.post<CreateTaskResponse>(
      '/api/v1/agent-orchestration/tasks',
      request
    );
  }
  ...
  async listTasks(request?: ListTasksRequest): Promise<ListTasksResponse> {
    ...
    return this.client.get<ListTasksResponse>(
      `/api/v1/agent-orchestration/tasks?${params.toString()}`
    );
  }
  ...
  async getTaskStatus(taskId: string): Promise<TaskStatusResponse> {
    return this.client.get<TaskStatusResponse>(
      `/api/v1/agent-orchestration/tasks/${taskId}/status`
    );
  }
  ...
  async executeTask(request: ExecuteTaskRequest): Promise<ExecuteTaskResponse> {
    return this.client.post<ExecuteTaskResponse>(
      `/api/v1/agent-orchestration/tasks/${request.task_id}/execute`,
   ...
   * ```
   */
  async createTaskSequence(request: CreateTaskSequenceRequest): Promise<CreateTaskSequenceResponse> {
    return this.client.post<CreateTaskSequenceResponse>(
      '/api/v1/agent-orchestration/sequences',
      request
    );
  }
}
